...
## DATA PREP
# Download training data from web and appending into a file on current directory
def download_text8(data_url="http://mattmahoney.net/dc/text8.zip", output_folder="data"):
    os.makedirs(output_folder, exist_ok=True)
    file_path = os.path.join(output_folder, "text8.zip")
    extracted_path = os.path.join(output_folder, "text8")
    ...
    for test_sample in test_data:
        ...
        if all(word in model.wv for word in [a, b, c, expected]):
            try:
                ...
                analogy_result = model.wv.most_similar(positive=[a, c], negative=[b], topn=1)[0][0]
                predicted = model.wv[a] - model.wv[b] + model.wv[c]
